...
func checkLdapConfiguration() {
	...
	verified = verifyAdminAccount(conn, adminaccounts)
...
func verifyAdminAccount(conn *ldap.Conn, adminaccounts [][]string) bool {
	...
	prompt := promptui.Prompt{
		Label: color.FgBlue.Sprintf("Enter password for %s",account),
		Mask:  '*',
	}
	...
	if err := conn.Bind(accountdn, accountpword); err != nil {
		color.Error.Printf("Failed to auth. %s", err)
		os.Exit(0)
		return false
	} else {
		color.Success.Println("Authenticated successfully!")
...
func checkConnectivity(a string, h string, port string) (int, int)  {
	...
	failures := 0
	successes := 0
	timeoutDuration := time.Duration(10) * time.Millisecond
	intervalDuration := time.Duration(1) * time.Millisecond
